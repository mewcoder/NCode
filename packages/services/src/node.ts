...
} from "./coding-plan-subscription/codingPlanSubscriptionService.js";
...
import { createDisabledCodingPlanSubscriptionService } from "./coding-plan-subscription/codingPlanSubscriptionService.js";
...
export function createLocalServices(options: {
  ...
  // 兼容旧 ServiceCollection 的 account source 形状，但不再读取 OAuth、账号身份或套餐权益。
  // Resolver 会进一步过滤 zhipu-account；该 source 只返回 fail-closed 事实，避免旧配置重新激活。
  const accountProviderConfigSource = createAccountProviderConfigSource({
    ...
    readSettings: async () => ({
      ...
      selections: {},
      unresolvedFamilies: ["zai", "bigmodel"] as const,
    }),
  ...
  const codingPlanSubscriptionService = createDisabledCodingPlanSubscriptionService();
  ...
  const zcodeAgentService = createZCodeAgentService({
    ...
    resolveSpawnEnv: async (context) => {
      ...
      // 遥测不再读取 OAuth 用户资料；设备级遥测仍可按原配置运行。
  ...
  const services = new ServiceCollection()
    ...
    .register(
      ...
      createUsageStatsService({
        ...
        resolveApiAuthorization: async ({ preferredProviderId }) => {
          ...
          const provider = providerRuntime.registryService
            .getSnapshot()
            ?.resolution.registryProviders.find((candidate) => candidate.providerId === providerId);
          const access = provider?.config.access;
          const baseUrl = provider?.config.api?.baseUrl?.trim();
          const apiKey = access?.type === "zhipu-coding-plan-api-key" ? access.apiKey?.trim() : "";
          ...
          if (!hostname.endsWith(".z.ai") && !hostname.endsWith(".bigmodel.cn")) {
            return null;
          }
          return {
            authorization: apiKey,
            quotaUrl: new URL("/api/monitor/usage/quota/limit", origin.origin).toString(),
            provider: {
              id: providerId,
              name: provider.providerName?.trim() || hostname,
            },
          };
        },
        zcodeAgentService,
