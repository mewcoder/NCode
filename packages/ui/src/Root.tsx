...
function RootInner({
  ...
  useEffect(() => {
    void Promise.all([refreshAppSettings(), refreshProviderState()]).catch((error) => {
      logger.warn("[Root] 启动刷新本地设置或 Provider 状态失败", { error });
    });
  }, [refreshAppSettings, refreshProviderState]);
  ...
  // 启动阻塞是桌面窗口保护期，Web 客户端在进入 Root 前已有加载页。
