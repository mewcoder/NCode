...
function waitForCuaApplicationReturn({
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const onQuit = () => finish(new Error("NCode quit during CUA permission onboarding"));
    const onAbort = () =>
      finish(signal.reason ?? new Error("CUA permission onboarding origin window closed"));
    observationTimer = setTimeout(
      () =>
        finish(
          new Error(`System Settings did not return to NCode within ${Math.max(1, timeoutMs)}ms`),
        ),
