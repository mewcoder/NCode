...
import { resolveLocalDesktopContextPromptEnabled } from "./localFeatureFlags.js";
...
// 内存诊断计数器：用于观察主进程内存占用和 will-download 监听是否残留。
...
app.whenReady().then(async () => {
  ...
  // 启动自动更新检查（后台执行，不阻塞主界面）。生产包从当前仓库的 GitHub Release
  // 获取稳定版更新；未打包开发构建的 generic feed 仅用于联调。
...
app.on("browser-window-created", (_, win) => {
  ...
  win.on("closed", () => {
    ...
    // 改为在窗口创建时缓存 webContents id，确保清理 deep link 状态时不再访问已销毁对象。
