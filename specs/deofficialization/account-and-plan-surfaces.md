...
购买和同步链路，使 ZCode 可以直接作为本地开发工具使用。保留通用 Provider、普通 API Key、
Coding Plan API Key、自定义模型和已有本地工作区能力。
...
Runtime 解析。普通 API Key/Coding Plan API Key Provider 不迁移。旧 OAuth 凭据、账号配置及少量共享
协议类型仍保留在磁盘/兼容层，不主动删除，也不进入新的 Provider/API Key 运行路径；`apps/zcode-cli`
保持原样。
