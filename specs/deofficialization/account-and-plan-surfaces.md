...
- Registry 使用 fail-closed 空账号源，不再装配账号连接 resolver；Provider Provisioning 只刷新本地
  Personal/Built-in Registry，不刷新账号套餐状态。
...
- Desktop、Web 和 UI 不再暴露官方账号登录、OAuth 回调、Token 刷新或官方账号登出命令；`apps/zcode-cli`
  按稳定边界保持不改动。
- Desktop、Web 和 UI 不再注册、调度或展示 Off-Peak/闲时任务；已有任务表和旧 CLI 协议类型不主动删除，
  但本地运行时不再读取、派发或向官方闲时端点发请求。
- `IOffPeakTaskService` 与 disabled 实现仅用于旧客户端协议协商；不再保留未实例化的任务 Repo、官方票据
  客户端、凭据解析、mock gateway 或这些实现的公开导出。
...
- 旧 OAuth 凭据和账号 Provider 配置不再被运行时读取；本次不主动删除磁盘凭据，但不保留官方账号接口或兼容服务。
...
13. `packages/services/src/node.ts` 只公开 Off-Peak 协议兼容接口，不再公开可连接官方闲时端点的实现；
    `pnpm dep:refs` 不应发现已删除 UI helper 的静态调用方。

## 迁移边界

本次移除 UI 入口、分享链路、Desktop/Web OAuth 回调桥接、构建注入的官方 OAuth 配置和账号 Provider
Runtime 解析，并移除 Off-Peak、官方 Server MCP 认证/额度和官方套餐购买/支付运行链路。普通 API
Key/Coding Plan API Key Provider 不迁移。旧 OAuth 凭据、账号配置、闲时任务数据及少量共享协议类型仍
保留在磁盘/兼容层，不主动删除，也不进入新的 Provider/API Key 运行路径；仅为历史实现服务且无调用方的
Repo、远端客户端、UI helper 和依赖应删除；`apps/zcode-cli` 保持原样。
